...
#check if the user has finished a particular exam

//$get_exam_status="SELECT * FROM exam_status";
//$get_exam_status_result=@mysqli_query($conn,$get_exam_status);
//
//
...
//         echo '<script>
//                    swal.fire({
//                        text:"An error occured, connection was abrupted",
//                        icon:"error",
//                        timer:3000
//
//                    });
//              </script>';
//        die("<h4 class='text-danger text-capitalize'>couldn't connect to server, connection down!</h4>");
//    }
//    
//    
//   #check if the query was successful
//    if(!$get_exam_status_result){
//        echo '<script>
//                    swal.fire({
//                        text:"An error occured, couldn`t get exam status info",
//                        icon:"error",
//                        timer:3000
//
//                    });
//              </script>';
//        die("<h4 class='text-danger text-capitalize'>an error occured while getting info!</h4>");
//    }else{
//      
//       $exam_status_row=@mysqli_fetch_assoc($get_exam_status_result);
//       
//       if($exam_status_row["exam_finished"] == 0){
//           
//           #the user didn't complete the former exam
//           echo '<script>
//               
//
//                swal.fire({
//                    title:"Exam-Incomplete!",
//                    text:"You have an outstanding exam to complete, you have to complete the exam before you can access another exam!",
//                    icon:"warning",
//                    allowOutsideClick:false,
//                    allowEscapeKey:false,
//                    confirmButtonText:"Complete The CBT",
...
<?php
    ...
    $subject_id=$_SESSION["s_id"];
    $get_questions_query="SELECT * FROM users_questions WHERE test_ID = '$test_id' ";
    $get_questions_result=@mysqli_query($conn,$get_questions_query);
      #get total question length
    $get_question_length_query="SELECT * FROM users_test WHERE subject_ID= '$subject_id' ";
    ...
    #check if there was an error 
            if(!$get_question_length_result){
                echo '<script>
                    swal.fire({
                        text:"An error occured, server retured 0 results while getting the question length!",
                        icon:"error",
                        timer:3000
            
                    });
                     </script>';
            echo("<h3 class='text-capitalize text-center text-danger'>An error occured </h3>".mysqli_error($conn));                 
  ...
  if(isset($_POST["next_question"]) AND $_SERVER["REQUEST_METHOD"] == serverMethod){
    ...
    if(isset($_POST["get_result"]) AND $_SERVER["REQUEST_METHOD"] == serverMethod){
                ...
                #change the test_status to finished
                
                $change_test_status="UPDATE exam_status SET exam_finished = 1";
                $change_test_status_result=@mysqli_query($conn,$change_test_status);
                
                #check if the query went well
                if(!$change_test_status_result){
                    echo mysqli_error($conn);
                    die("<h4 class='text-danger text-capitalize'>an error occured, could not update test status</h4>");
                }else{
                        unset($_SESSION["question_number"]);
                        unset($_SESSION["true_answer"]);
                        unset($_SESSION["t_id"]);
                        unset($_SESSION["s_id"]);
                        $_SESSION["taking_test"]=false;
                        unset($_SESSION["taking_test"]);
            ...
            if(isset($test_ID) && isset($sub_ID)){
                ...
                #navigate the user to the same page
                echo '<script>
                    location.href="CBT.php";
                      </script>';
...
if(!$get_questions_result){
    ...
    if($_SESSION["question_number"] > 0){
            #update the test status the dbase
