...
<section class="text-capitalize p-4 m-auto text-center">
    ...
    <img src="<?php echo "../profilePic/".$admin_info_row["profile_picture"]?>" height="50" width="50" class="rounded-circle" alt="<?php echo $admin_info_row["user_name"]. " " . "profile picture"?>"/>
    <h2 class="text-capitalize text-center p-3" >welcome <?php echo $row["user_name"]?></h2>
</div>
dear <?php echo "Admin,". $row["user_name"]?>, change your profile information below, choose the profile information type you want to change and proceed below
</section>

<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"])?>" class="m-auto  p-5 form rounded shadow form-group w-100 bg-dark" method="POST" enctype="multipart/form-data ">
<h3 class="text-capitalize text-center">Admin, change your profile details</h3>
<br/>
...
<br/>
<input type="email" placeholder="Update Your E-mail" class="form-control text-light" name="email" required autofocus value="<?php echo $admin_info_row["email"] ?>"/>
<br/>

<label for="homeAddress" class="text-capitalize ">Home-Address:</label>
<br/>
<input type="text" placeholder="Update Your Home-Address" class="form-control text-light " name="homeAddress" required value="<?php echo $admin_info_row["home_address"] ?>"/>
<br/>

<label for="phoneNumber" class="text-capitalize ">Phone-Number</label>
<br/>
<input type="number" placeholder="Update Your Phone-Number" class="form-control text-light" name="phoneNumber" required value="<?php echo $admin_info_row["phone_number"] ?>"/>
<br/>

<label for="birthday" class="text-capitalize ">Birthday:</label>
<br/>
<input type="date" placeholder="Update Your Birthdate" class="form-control text-light " name="birthday" required value="<?php echo $admin_info_row["birthday"] ?>"/>
<br/>

<label for="comment" class="text-capitalize ">Update Your Bio:</label>
<br/>
<textarea placeholder="Update Your Comment" class="form-control text-capitalize text-light bg-dark" name="comment" required>
...
<input type="radio" value="male" class="d-inline radio-btn" name="gender" required /> <p class="text-capitalize d-inline text-light">male</p>
<br/>
<input type="radio" value="female" class="d-inline radio-btn" name="gender" required /> <p class="text-capitalize d-inline text-light">Female</p>

<br/>
<hr/>
<h5 for="profilePic" class="text-capitalize text-bold">profile picture</h5>
<br/>
<input type="file" value="profilePic" class="form-control-file border bg-secondary p-2 text-light w-75" name="profilePic" id="profilePic" required>
<br/>
<hr/>

<button class="btn btn-danger text-light form-control text-center text-capitalize" name="ad_submit"> update profile</button>
<br/>
<br/>
...
if(isset($_POST["ad_submit"]) and  $_SERVER["REQUEST_METHOD"]==serverMethod){
  ...
  if(!empty($email) and !empty($homeAddress) and !empty($phoneNumber) and !empty($birthday) and !empty($bio) and !empty($sex) and !empty($profilePic)){
    ...
    if(!$conn){
      ...
      $admin_exist_query="SELECT * FROM admin_info WHERE A_ID = $ID";
      $admin_exist_result=mysqli_query($conn,$admin_exist_query);

      if(!$admin_exist_result){
        #an error occurred during the process
        unknown_error_msg();
      }else{
        if(mysqli_num_rows($admin_exist_result) ==1){
          #the user exist
          ...
          #check if the user image extension matches with the legit extension
          if(in_array($legit_ext,$legit_file_ext_array)){
            ...
            $update_profile_query="UPDATE admin_info SET email ='$email', birthday ='$birthday', sex ='$sex', comment ='$bio', home_address ='$homeAddress', phone_number ='$phoneNumber', profile_picture ='$new_file_name', time_updated ='$timeUpdated' WHERE A_ID = '$ID'";
