...
<br/>
<h3 class="text-capitalize p-2 text-center">search for a user to chat with...</h3>
<br/>
...
<div class="search_div">
 <form class="search-area m-auto p-4 d-flex justify-content-center align-items-center flex-column form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"])?>">
...
<br/><br/>
    <input type="submit" value="Search User" class="btn btn-danger   form-control text-center" name="users_search">
...
<br/>
    <input type="submit" value="Search User" class="btn btn-danger form-control text-center" name="submit_direct_query">
    <br/>
       ...
       #check the type of request the user is tying to make
        switch ($user_selected_item) {
            ...
            case '0':
             ...
             $select_query="SELECT * FROM users_info  WHERE NOT U_ID ='$ID' ";

                break;

             case '2':
                # get all the old users from the dbase
             $select_query="SELECT * FROM users_info WHERE NOT U_ID ='$ID' ORDER BY U_ID ASC";
                break;

             case '3':
                #get all the active users from the dbase
             $select_query="SELECT * FROM users_info WHERE NOT U_ID ='$ID' AND status ='$default_status'";
                break;

             case '4':
            # get all the old users from the dbase
             $select_query="SELECT * FROM users_info WHERE NOT U_ID ='$ID' ORDER BY U_ID DESC";
                 break;
            default:
         ...
         #check if something went wrong with the connection
            ...
            if(!$get_users_result){
                ...
                while($user_search_row=mysqli_fetch_assoc($get_users_result)){
                    ...
                    <a href="adminMessageUser.php?receiver_id='.htmlspecialchars($user_search_row["U_ID"]).'"title="View '.$user_search_row["user_name"].' Profile" class="d-block text-decoration-none" autofocus style="border:none; outline:none; ::selection{border:none; outline:none;}">
                    <div class=" container" >
        ...
        if(empty($user_search_order) or $user_search_order == 0){
           ...
           switch ($user_search_order) {
            ...
            if(!$user_search_result){
                ...
                } if(mysqli_num_rows($admin_query_result) > 0){
