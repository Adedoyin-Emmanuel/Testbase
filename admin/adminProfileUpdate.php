...
if(!empty($_SESSION["Admin_ID"])){
	...
	$query3="SELECT * FROM admin_info WHERE A_ID = $ID ";
	...
	if(mysqli_num_rows($result3) > 0){
		...
		#check if the profile is completed
			...
			if($row3["profile_updated"] == 1){
				...
				//echo "baba you haven't completed any profile jare ";
			}
...
<br/>
<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"])?>" class="m-auto w-75 p-4 form rounded shadow" method="POST" enctype="multipart/form-data">
<h5 class="text-capitalize text-center bg-success p-1 text-light">Admin profile update</h5>
<br/>
...
<br/>
<input type="text" placeholder="Username already stored in server" class="form-control" readonly/>
<br/>
<label for="password" class="text-capitalize ">password:</label>
<br/>
<input type="" placeholder="Password already stored in server" class="form-control"  readonly/>
<br/>

<label for="email" class="text-capitalize ">E-mail:</label>
<br/>
<input type="email" placeholder="Enter Your E-mail" class="form-control" name="email" required autofocus/>
<br/>

<label for="homeAddress" class="text-capitalize ">Home-Address:</label>
<br/>
<input type="text" placeholder="Enter Your Home-Address" class="form-control" name="homeAddress" required/>
<br/>

<label for="phoneNumber" class="text-capitalize ">Phone-Number</label>
<br/>
<input type="number" placeholder="Enter Your Phone-Number" class="form-control" name="phoneNumber" required/>
<br/>

<label for="birthday" class="text-capitalize ">Birthday:</label>
<br/>
<input type="date" placeholder="Enter Your Birthdate" class="form-control" name="birthday" required/>
<br/>

<label for="comment" class="text-capitalize ">Let's know a little about you:</label>
<br/>
<textarea placeholder="write something" class="form-control text-capitalize" name="comment" required>
    </textarea>
...
<br/>
<input type="file" value="profilePic" class="bg-dark p-2 text-light w-75" name="profilePic" required>
<br/>
<hr/>

<button class="btn btn-success form-control text-center text-capitalize" name="ad_submit"> update admin profile</button>
<br/>
<br/>
...
if(isset($_POST["ad_submit"]) and $_SERVER["REQUEST_METHOD"]==serverMethod){
	...
	if(!empty($email) and !empty($phoneNumber) and !empty($homeAddress) and !empty($birthday) and !empty($comment) and !empty($sex) and !empty($profilePic)){
		...
		if($upload_file_error != 0){
			die("an error occured with the file in process");
			unknown_file_error();
		}else{
		...
		if(!$conn){
			...
			if(mysqli_num_rows($result) > 0){		
				$query2="SELECT * FROM admin WHERE ID ='$ID' AND  user_name ='$userName' AND password ='$password' ";
			    ...
			    $newQuery="UPDATE admin_info SET email ='$email', birthday ='$birthday', sex ='$sex', comment ='$comment', home_address ='$homeAddress', phone_number ='$phoneNumber', profile_picture ='$processed_file_name', time_updated ='$date_time_updated', user_name ='$userName', password ='$password', profile_updated='$profile_updated' WHERE A_ID = '$ID'";
