...
if(isset($chat_admin_receiver_id)){
   $_SESSION["chat_admin_receiver_id"] = $chat_admin_receiver_id;
   echo '<script>
    location.href="messageAdmin.php";
   </script>';

}

if(!isset($_SESSION["chat_admin_receiver_id"])){
  echo '<script>
   location.href="showAdmins.php";
   </script>';
}
...
<script src="getAdminStatus.js"></script>
...
<div class="container d-flex justify-content-around align-items-start flex-column text-capitalize">
    ...
    <img src="images/arrow_down_2.png" height="20" width="20" alt="down_scroll_button" class="text-dark"/>
</div>
...
$(document).ready(()=>{
  setInterval(()=>{
    ...
    xhttp.open("GET","showAdminMessages.php",true);
    xhttp.send();
...
<input type="text" name="receiver" id="receiver" hidden value="<?php echo $_SESSION['chat_admin_receiver_id']?>">
...
<div class="d-flex align-items-center justify-content-center flex-wrap flex-column m-auto" id="focusElement">
  ...
  <br/>
  <button class="btn btn-danger"><a href="showUsers.php" class="text-center text-capitalize text-light text-decoration-none"> &lt;&lt;  back</a></button>
  ...
  $(document).ready(()=>{
    $("form").submit((e)=>{
       ...
       */
       ...
       xhttp.open("POST",`saveAdminMsg.php?msg=${typed_msg}`,true);
       //send the request
